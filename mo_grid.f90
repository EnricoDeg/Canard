...
MODULE mo_grid
   ...
   SUBROUTINE calc_grid_metrics
      ...
      do m=1,3
         rr(:,1)=xim(:,m)
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+1)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(1))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+2)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(2))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+3)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(3))
         xim(:,m)=rr(:,1)
         rr(:,1)=etm(:,m)
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+4)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(1))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+5)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(2))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+6)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(3))
         etm(:,m)=rr(:,1)
         rr(:,1)=zem(:,m)
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+7)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(1))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+8)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(2))
         call mpigo(rr(:,1), ijk, nbc, mcd, nbsize,1,nrone,n45no,9*(m-1)+9)
         call filte(rr(:,1), lxi, let, lze, ijk, nnf(3))
         zem(:,m)=rr(:,1)
      end do
