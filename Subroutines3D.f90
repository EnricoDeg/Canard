...
!===== SUBROUTINE FOR MPI IMPLEMENTATION
 ...
 select case(nn)
 case(1); send=>send11; recv=>recv11; case(2); send=>send12; recv=>recv12; case(3); send=>send13; recv=>recv13
 end select
 end if
 ...
 if(nt==0) then
    ir=ir+1; call MPI_ISEND(send(:,:,ip),2*nbsize(nn),MPI_REAL8,mcd(nn,ip),itag+iq,icom,ireq(ir),ierr)
    ir=ir+1; call MPI_IRECV(recv(:,:,ip),2*nbsize(nn),MPI_REAL8,mcd(nn,ip),itag+ip,icom,ireq(ir),ierr)
 else
    ir=ir+1; call MPI_ISEND(send(:,:,ip),3*nbsize(nn),MPI_REAL8,mcd(nn,ip),itag+iq,icom,ireq(ir),ierr)
    ir=ir+1; call MPI_IRECV(recv(:,:,ip),3*nbsize(nn),MPI_REAL8,mcd(nn,ip),itag+ip,icom,ireq(ir),ierr)
 end if
 end if
 end do
 end do
 ...
 do nn=1,3; nz=(1-nrt)*(nn-1)+1
 if(nt==0) then
 select case(nn); case(1); recv=>recv01; case(2); recv=>recv02; case(3); recv=>recv03; end select
 else
 select case(nn); case(1); recv=>recv11; case(2); recv=>recv12; case(3); recv=>recv13; end select
 end if
...
!===== SUBROUTINE FOR COMPACT FINITE DIFFERENTIATING
 ...
 integer(kind=ni),intent(in) :: nn,nz,m
    ...
    nt=0; ns=ndf(nn,0,0); ne=ndf(nn,1,0)
...
!===== SUBROUTINE FOR COMPACT FILTERING
 ...
 integer(kind=ni),intent(in) :: nn,nz
    ...
    nt=1; ns=ndf(nn,0,1); ne=ndf(nn,1,1)
 ...
 do i=is+3,ie-3
    res=sa(i)+sa(i); sb(i)=fa*(sa(i-1)+sa(i+1)-res)+fb*(sa(i-2)+sa(i+2)-res)+fc*(sa(i-3)+sa(i+3)-res)
 end do
 select case(ne)
 case(0)
