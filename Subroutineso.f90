...
!    ao=log(two); res=(fltkbc-fltk)/9; ra2=fltk+res; ra1=fltk+4*res; ra0=fltk+9*res
!
!    call fcint(ra0,half,alphz,betz,za,zb,zc); fctr=one/(one+alphz/2+betz/4)
!    albef(:,0,0)=(/zero,zero,one,alphz*fctr,betz*fctr/); res=(4*za+6*zb+7*zc)/ao
!    fbc(:,0)=(/za-5*res/24,zb+5*res/84,zc-5*res/336,5*res/2016,-res/5040/)*fctr
!
!    call fcint(ra1,half,alphz,betz,za,zb,zc)
!    albef(:,1,0)=(/zero,alphz+betz/2,one,alphz,betz/); res=(2*zb+3*zc)/ao
!    fbc(:,1)=(/za+zb+zc+1627*res/5040,za+5*res/42,zb-5*res/168,zc+5*res/1008,-res/2520/)
!
!    call fcint(ra2,half,alphz,betz,za,zb,zc)
!    albef(:,2,0)=(/betz,alphz,one,alphz,betz/); res=zc/ao
!    fbc(:,2)=(/zb+zc+1627*res/2520,za-5*res/6,za-5*res/84,zb+5*res/504,zc-res/1260/)
